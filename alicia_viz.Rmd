...
avg_hilow <- ggplot(denali_agg, 
       aes(x=year, tmax))+ 
  geom_point(col="darkorange3")+
  scale_y_continuous(limits=c(min(denali_agg$tmin), max(denali_agg$tmax)))+
  geom_point(data=denali_agg, aes(x=year, tmin), col="aquamarine4")+
  stat_smooth(method="lm", col="firebrick4")+
  stat_smooth(data=denali_agg, aes(x=year,tmin), col="dodgerblue4", method="lm") +
  theme_minimal() +
   theme(plot.title = element_text(hjust = 0.5, size = 22, margin=margin(0,0,15,0)),
        plot.subtitle = element_text(hjust = 0.5, size = 18, margin=margin(0,0,15,0)),
  ...
  labs(y="Average Temperatures (°F) ", x = "Year", title = "Average High and Low Temperatures per Year", subtitle = "(1923–2020)")

avg_hilow

ggsave(here("images","avg_hilow.jpg"), height = 6, width = 9)
```


```{r annual, echo=TRUE}
##### AVERAGE TEMP (tmin + tmax divided by 2)
avg_temp <- ggplot(denali_agg, 
       aes(x=year, mean))+
  geom_point(data=denali_agg, aes(x=year, mean), col="cornflowerblue")+
  stat_smooth(data=denali_agg, aes(x=year,mean), col="mediumpurple4", method="lm") +
  theme_minimal() +
  labs(y="Average Temperature (°F) ", x = "Year", title = "Average Temperatures per Year", subtitle = "(1923–2020)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 22, margin=margin(0,0,15,0)),
        plot.subtitle = element_text(hjust = 0.5, size = 18, margin=margin(0,0,15,0)),
        axis.text.x = element_text(size = 16, angle = 0, hjust = .5, vjust = .5),
...
ggsave(here("images","avg_temp.jpg"), height = 6, width = 9)
...
```{r}
##### Precipitation 
precip <- ggplot(denali_agg, 
       aes(x=year, precip))+
  geom_point(data=denali_agg, aes(x=year, precip), col="cornflowerblue")+
  stat_smooth(data=denali_agg, aes(x=year,precip), col="mediumpurple4", method="lm") +
  theme_minimal() +
   labs(y="Annual Precipitation (Inches)", x = "Year", title = "Total Annual Precipitation", subtitle = "(1923–2020)") +
  theme(plot.title = element_text(hjust = 0.5, size = 22, margin=margin(0,0,15,0)),
        plot.subtitle = element_text(hjust = 0.5, size = 18, margin=margin(0,0,15,0)),
        axis.text.x = element_text(size = 16, angle = 0, hjust = .5, vjust = .5),
        axis.text.y = element_text(size = 16, angle = 0, hjust = 1, vjust = 0),  
...
ggsave(here("images","precip.jpg"), height = 6, width = 9)

##### combine avg temps with precip 

temps_and_precip <- (avg_hilow | precip)

temps_and_precip

ggsave(here("images","temps_and_precip.jpg"), height = 6, width = 14)
...
### LM for precip 
res=lm(precip~year, data=denali_agg)
summary(res)
```
...
freeze_days <- ggplot(denali_data.pk, 
       aes(x=year, y=ndayfr)) + 
  stat_summary(fun.y="mean", geom="point", col="aquamarine4", size=2)+
  theme(axis.text=element_text(size=14, face="bold"), 
        axis.title=element_text(size=14, face="bold")) + 
  geom_smooth(method="lm", col = "dodgerblue4") +
  labs(y="Number of Freezing Days per Year", x = "Year", title = "Number of Freezing Days per Year",subtitle = "(1944–2020)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 22, margin=margin(0,0,15,0)),
        plot.subtitle = element_text(hjust = 0.5, size = 18, margin=margin(0,0,15,0)),
        axis.text.x = element_text(size = 16, angle = 0, hjust = .5, vjust = .5),
        ...
        axis.title.x = element_text(size = 18, angle = 0, hjust = .5, vjust = 0, margin=margin(10,0,0,0)),
        axis.title.y = element_text(size = 18, angle = 90, hjust = .5, vjust = .5, margin=margin(0,10,0,0))) +
  scale_x_continuous(breaks = c(1944, 1960, 1980, 2000, 2020))

freeze_days
ggsave(here("images","freeze_days.jpg"), height = 6, width = 10)
```
...
hot_days <- ggplot(denali.hot, aes(x=year, y=hotday)) + 
  stat_summary(fun.y="mean", geom="point", col="darkorange3", size=2)+
  ...
  labs(y="Recorded Temperature (°F) ", x = "Year", title = "Hottest Temperatures per Year", subtitle = "(1930–2019)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 22, margin=margin(0,0,15,0)),
        plot.subtitle = element_text(hjust = 0.5, size = 18, margin=margin(0,0,15,0)),
        axis.text.x = element_text(size = 16, angle = 0, hjust = .5, vjust = .5),
...
hot_days
ggsave(here("images","hot_days.jpg"), height = 6, width = 10)

## combine using patchwork 

combined <- (freeze_days | hot_days) 

combined

ggsave(here("images","combined.jpg"), height = 6, width = 14)
